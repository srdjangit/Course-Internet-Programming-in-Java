...
public class IsAdminCheck implements PhaseListener {
    ...
    public void afterPhase(PhaseEvent event) {
        ...
        boolean moderatorPage = fc.getViewRoot().getViewId().lastIndexOf("/app-moderator/") > -1 ? true : false;
        boolean userPage = fc.getViewRoot().getViewId().lastIndexOf("/app-user/") > -1 ? true : false;
        // Da li je logovan kao admin ?
        UserController uc = (UserController) getBean("userController");
        if (adminPage && !uc.getLoggedUser().getRole().getName().equalsIgnoreCase("admin")) {
            NavigationHandler nh = fc.getApplication().getNavigationHandler();
            nh.handleNavigation(fc, null, "adminLoginFalse");
            System.out.println("Nedozvoljen prilaz");
        }
        else  if (moderatorPage && !uc.getLoggedUser().getRole().getName().equalsIgnoreCase("moderator")) {
            NavigationHandler nh = fc.getApplication().getNavigationHandler();
            nh.handleNavigation(fc, null, "adminLoginFalse");
            System.out.println("Nedozvoljen prilaz");
        }
        else  if (userPage && !uc.getLoggedUser().getRole().getName().equalsIgnoreCase("user")) {
            NavigationHandler nh = fc.getApplication().getNavigationHandler();
            nh.handleNavigation(fc, null, "adminLoginFalse");
            System.out.println("Nedozvoljen prilaz");
        }
